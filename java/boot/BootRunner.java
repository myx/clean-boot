...
final class BootRunner extends Thread {
		...
		}
		{
			...
			if (data != null) {
				...
				for (final Enumeration<?> names = local.propertyNames(); names.hasMoreElements();) {
					...
					if (result.getProperty(current, "").isBlank()) {
	...
	BootRunner(final String[] args, final File rootProtected, final File rootPublic) throws Exception {
		...
		{
			assert rootProtected.exists() || rootProtected.mkdirs() : "Cannot create directory: path=" + rootProtected.getAbsolutePath();
			...
			/* if (zip) { this.source = new SourceZip( this.current ); } else { final SourceFiles
			 * files = new SourceFiles( this.current ); this.source = new SourceDirectImpl(
			 * files.list(), files ); } */
		}
		{
			final ClassLoader current = BootRunner.class.getClassLoader();
			this.bootClassLoader = new LdCore(new LdLibs(new File(this.rootProtected, "axiom"), new File(this.rootPublic, "axiom"), current), this.source);
